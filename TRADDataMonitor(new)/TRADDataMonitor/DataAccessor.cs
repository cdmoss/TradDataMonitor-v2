            ...
            try
            {
                ...
                using (SQLiteDataReader reader = cmd.ExecuteReader())
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        double data = Convert.ToDouble(reader.GetString(1).Split(' ')[0]); // need to compensate for VOC at some point
                        DateTime date = DateTime.ParseExact(reader.GetString(2), "MM-dd-yyyy HH:mm:ss", CultureInfo.InvariantCulture);
                        string serial = reader.GetString(3);
                        string hubName = reader.GetString(4);
                        dr["SensorType"] = type;
                        dr["Data"] = data;
                        dr["DateTime"] = date;
                        dr["SerialNumber"] = serial;
                        dr["HubName"] = hubName;
